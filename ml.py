...
# Create a dataframe with the two lists of skills as columns
df = pd.DataFrame({'team_player': team_player_skills, 'team_leader': team_leader_skills})

# Create a count vectorizer object
vectorizer = CountVectorizer()

# Fit and transform the text data to create a document-term matrix
doc_term_matrix = vectorizer.fit_transform(df.values.flatten())

# Split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(doc_term_matrix, df.columns, test_size=0.2, random_state=42)
...
descriptionA_vectorized = vectorizer.transform([descriptionA])
prediction = model.predict(descriptionA_vectorized)[0]
